...
% Load Data
edge_attr = load('edge_attr.mat'); % Edge features
edge_index = load('edge_index.mat'); % Edge index (to make A)
X = load('X.mat'); % Node features
Y = load('Y_polar.mat'); % Node labels
...
maxSnapshots = 100; % Define the maximum number of snapshots to use
...
% Initialize weights
W1 = initializeWeights(size(X.Xpf{1}, 2), hidden_dim);
W2 = initializeWeights(hidden_dim, hidden_dim);
W3 = initializeWeights(hidden_dim, output_dim);
...
for epoch = 1:num_epochs
    ...
    for i = 1:numel(X_train)
        ...
        % Forward pass
        Y_pred = GINEConv(ANorm, X, E, W1, W2, W3);
        ...
        [grad_W1, grad_W2, grad_W3] = computeGradients(ANorm, X, E, W1, W2, W3, Y);
        W1 = W1 - lr * grad_W1;
        W2 = W2 - lr * grad_W2;
        W3 = W3 - lr * grad_W3;
    end
    ...
    % Average Loss
    avg_loss = total_loss / numel(X_train);
    ...
    if mod(epoch, 10) == 0
        ...
        for i = 1:numel(X_val)
            X = X_val{i};
            Y = Y_val{i};
            Y_val_pred = GINEConv(ANorm, X, E, W1, W2, W3);
            val_loss = val_loss + mean((Y_val_pred - Y).^2, 'all');
        end
        val_loss = val_loss / numel(X_val);
...
for i = 1:numel(X_test)
    X = X_test{i};
    Y = Y_test{i};
    Y_test_pred = GINEConv(ANorm, X, E, W1, W2, W3);
    test_loss = test_loss + mean((Y_test_pred - Y).^2, 'all');
end
test_loss = test_loss / numel(X_test);
...
function Y = GINEConv(ANorm, Z1, E, W1, W2, W3)
    % Inputs:
    % ANorm: normalized adjacency matrix (N x N)
    ...
    % Layer 1
    Z2 = GINEConvLayer(ANorm, Z1, E, W1);

    % Layer 2
    Z3 = GINEConvLayer(ANorm, Z2, E, W2);

    % Layer 3
    Z4 = GINEConvLayer(ANorm, Z3, E, W3);
...
function [grad_W1, grad_W2, grad_W3] = computeGradients(ANorm, X, E, W1, W2, W3, Y_true)
    % Compute Gradients for GINEConv Layers

    gradients = dlgradients(loss, parameters)

    % Forward pass
    Z1 = GINEConvLayer(ANorm, X, E, W1);
    Z2 = GINEConvLayer(ANorm, Z1, E, W2);
    Z3 = GINEConvLayer(ANorm, Z2, E, W3);
    Y_pred = Z3;

    % Loss gradient (MSE Loss)
    dLoss_dY = 2 * (Y_pred - Y_true) / numel(Y_true);

    % Backpropagation through W3
    grad_W3 = (Z2' * dLoss_dY); % Gradient of W3
    dLoss_dZ2 = (dLoss_dY * W3') .* (Z2 > 0); % ReLU derivative

    % Backpropagation through W2
    grad_W2 = (Z1' * dLoss_dZ2); % Gradient of W2
    dLoss_dZ1 = (dLoss_dZ2 * W2') .* (Z1 > 0); % ReLU derivative

    % Backpropagation through W1
    grad_W1 = (X' * dLoss_dZ1); % Gradient of W1
end
